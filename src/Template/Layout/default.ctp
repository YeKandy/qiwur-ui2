...
<head>
  ...
  <title>齐物数据引擎 - 格物，齐物，用物</title>
	...
	<?php if(!$debugCss) : ?>
	<?php echo $this->element('css', ['css' => ['foundation.min', 'wufoo', 'qiwu-ui2']]); ?>
    <?php else : ?>
...
<body id="<?=$this->request->params['controller'].'-'.$this->request->params['action'] ?>">
  ...
  <div id="nav">
    ...
    <ul id="menu" class="clearfix">
      ...
      <li>
        <a href="<?php echo Router::url('/page-entities') ?>">Page Entities</a>
      </li>
      <li>
...
<?php 
	$scripts_for_layout = '';
	echo $this->element('js', ['scripts_for_layout' => $scripts_for_layout]);
?>
